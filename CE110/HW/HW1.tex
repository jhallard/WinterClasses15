...
% $$\text{Given } k \in \mathbb{R}^+ : $$ 
 ...
 \begin{center}
 \begin{tabular}{| l || r |}
 \hline Near-Threshold  & Super-Threshold \\ \hline
 $P_{nth} = V_{nth}^3 = k^3$ & $P_sth = V_{sth}^3 = (2k)^3 = 8k$ \\[0.15in]
 $D_{nth} = \frac{1}{V_{nth}} = \frac{1}{k}$ & $D_{sth} = \frac{1}{2k}$ \\[0.15in] 
 $E_{nth} = P_{nth}*D_{nth} = \frac{k^3}{k} = {k}^2$ & $E_{sth} = P_{sth}*D_{sth} = \frac{8k^3}{2k} = {4k}^2$ \\[0.15in]
 $EE_{nth} = E_{nth}*D_{nth} = \frac{k^2}{k} = k$ &  $EE_{sth} = E_{sth}*D_{sth} = \frac{4k^2}{2k} = 2k$ \\[0.05in] \hline
 \end{tabular}
\end{center}
 \end {table}
